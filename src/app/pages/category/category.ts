...
export class Category {
  ...
  openAdd(): void {
    ...
    this.form.reset({ name: '', description: '', gstRate: 18, defaultHsnCode: '', taxInclusive: true, status: 'Active' });
  }

  openEdit(category: CategoryRecord): void {
    this.editingId.set(category.id);
    this.form.reset({ name: category.name, description: category.description, gstRate: category.gstRate, defaultHsnCode: category.defaultHsnCode, taxInclusive: category.taxInclusive, status: category.status });
  }
  ...
  save(): void {
    ...
    const value = this.form.getRawValue() as { name: string; description: string; gstRate: number; defaultHsnCode: string; taxInclusive: boolean; status: 'Active' | 'Inactive' };

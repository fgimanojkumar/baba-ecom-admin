...
export class State {
  ...
  readonly form = this.fb.group({
    ...
    deliveryMessage: ['Delivery is temporarily unavailable in this state.'],
  });

  openAdd(): void {
    this.editingId.set(null);
    this.form.reset({ name: '', code: '', status: 'Active', deliveryMessage: 'Delivery is temporarily unavailable in this state.' });
  }

  openEdit(state: StateRecord): void {
    this.editingId.set(state.id);
    this.form.reset({ name: state.name, code: state.code, status: state.status, deliveryMessage: state.deliveryMessage ?? 'Delivery is temporarily unavailable in this state.' });
  }
  ...
  save(): void {
    ...
    const value = this.form.getRawValue() as { name: string; code: string; status: 'Active' | 'Inactive'; deliveryMessage: string };

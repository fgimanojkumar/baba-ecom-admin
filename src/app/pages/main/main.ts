...
export class Main {
  ...
  private readonly allNavItems: NavItem[] = [
    { label: 'Super Admin Dashboard', route: '/main/dashboard', icon: 'bi-speedometer2', roles: ['admin'] },
    { label: 'Business Dashboard', route: '/main/seller-dashboard', icon: 'bi-activity', permission: 'Business Dashboard' },
    { label: 'Reports', route: '/main/reports', icon: 'bi-graph-up-arrow', roles: ['admin'] },
    { label: 'GST & Tax Master', route: '/main/tax-master', icon: 'bi-receipt-cutoff', roles: ['admin'] },
    ...
    { label: 'AI Auto Listing', route: '/main/auto-listing', icon: 'bi-stars' },
    ...
    { label: 'GST Compliance', route: '/main/gst-compliance', icon: 'bi-file-earmark-check', roles: ['seller', 'finance'] },
    ...
    { label: 'Pincodes', route: '/main/pincodes', icon: 'bi-pin-map', roles: ['admin'] },
  ...
  readonly navItems = computed(() => {
    ...
    const roleLabel = this.authService.currentUser()?.roleLabel;
    return this.allNavItems.filter((item) => {
      const matchesLegacyRole = !item.roles || (role && item.roles.includes(role));
      const matchesPermission = !item.permission || this.roleService.hasPermission(roleLabel, item.permission);
      return matchesLegacyRole && matchesPermission;
    });
  });

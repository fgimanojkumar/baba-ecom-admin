...
import { PaymentRecord, PaymentService, PaymentStatus } from '../../shared/payment.service';
...
export class Payments {
  ...
  onPaymentAction(id: string, status: PaymentStatus): void {
    this.updateStatus(id, status);
    if (this.selectedPayment()?.id === id) {
      this.selectedPayment.set(this.payments().find((payment) => payment.id === id) ?? null);
    }
  }

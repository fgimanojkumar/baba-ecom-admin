...
import { SellerChangeRequest, SellerRecord, SellerService, SellerStatus } from '../../shared/seller.service';
...
export class Sellers {
  ...
  readonly changeRequests = this.sellerService.changeRequests;
  readonly selectedChangeRequest = signal<SellerChangeRequest | null>(null);
  ...
  viewChangeRequest(request: SellerChangeRequest): void {
    this.selectedChangeRequest.set(request);
  }

  approveChangeRequest(request: SellerChangeRequest): void {
    const resolved = this.sellerService.resolveChangeRequest(request.id, 'Approved');
    if (resolved?.bankDetails && resolved.sellerId === 'SEL001') {
      this.bankDetailsService.updateBankDetails(resolved.bankDetails);
    }
    this.selectedChangeRequest.set(resolved);
  }

  rejectChangeRequest(request: SellerChangeRequest): void {
    this.selectedChangeRequest.set(this.sellerService.resolveChangeRequest(request.id, 'Rejected', 'Details could not be verified. Please submit valid documents.'));
  }
}
